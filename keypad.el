...
(defun keypad--handle-input-key (key)
  (cond ((and (keypad--C-x-or-C-c?)
         ...
         (setq keypad--pending-modifier (if (or (null keypad--pending-modifier)
                                                (eq 'literal keypad--pending-modifier))
                                            'control
                                          'literal)))
        (keypad--pending-modifier
         (let ((k (pcase keypad--pending-modifier
                    ('control      (keypad--add-control key))
                    ('meta         (keypad--add-meta key))
                    ('control-meta (keypad--add-control-meta key))
                    ('literal      key))))
           (push k keypad--keys)
           (setq keypad--pending-modifier (cond ((eq keypad--pending-modifier 'literal) 'literal)
...
that were entered in the Keypad."
  (when (and which-key-mode
    ...
    (when-let* ((keymap (keypad--keymap-for-preview)))
        ...
        (map-keymap #'(lambda (event command)
                        (unless (eq command 'digit-argument)
                          (when-let* ((key (single-key-description event))
                                      ((key-valid-p key))
                                      ((not (keypad--keypad-self-key-p key)))
...
(defun keypad--keypad-self-key-p (key)
