...
`helix-search--function' returns current searching REGEXP.")

(defvar helix-search--regions-overlays nil
  "List of overlays for regions from `helix-search--regions' list.")
    ...
    (condition-case nil
        (let ((regexp (helix-read-pcre-regex "select: ")))
          (if (string-empty-p regexp)
            ...
            ;; else
            (set-register "/" regexp)
            (helix-create-cursors helix-search--regions)
...
(defun helix--search-highlight (_ _ _)
  ...
  (let ((regex (minibuffer-contents))
        result)
    (with-minibuffer-selected-window
      (setq helix-search--regions (funcall helix-search--function regex))
