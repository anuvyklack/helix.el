...
(defvar keypad-keymaps-to-ingnore-for-transparent-leader
  (list meow-normal-state-keymap meow-motion-state-keymap))
  ...
  (when-let* ((key (single-key-description input-event)))
    (let ((meta? (keypad--meta-keybindings-available-p)))
      (cond (keypad--modifier
            ...
            ((when-let* ((input (alist-get key keypad-start-keys)))
...
to Which-key API."
  (let* ((input (keypad--get-entered-keys-as-string))
         (meta-keymap (keypad--lookup-key (kbd (concat input " ESC"))
                                          ;; (read-kbd-macro (concat input " ESC"))
                                          )))
    (pcase keypad--modifier
      ('meta (if meta-keymap
                 (keypad--filter-keymap meta-keymap)))
      ('control-meta (if meta-keymap
                         (keypad--filter-keymap meta-keymap :control)))
      ('literal (if-let* ((keymap (keypad--lookup-key (kbd input))))
                    (if (keymapp keymap)
                        (keypad--filter-keymap keymap)))))))

;; (single-key-description 'lol)
;; (event-basic-type 'C-del)
;; (event-basic-type 'C-tab)

(defun keypad--keymap-to-describe-entered-keys-with-modifier-2 ()
  "Return a keymap with continuations for prefix keys + modifiers
entered in Keypad. This keymap is intended to be passed further
to Which-key API."
  (let* ((input (keypad--get-entered-keys-as-string))
         (ctrl-predicate (lambda (basic-key modifiers)
                           (not (or (equal basic-key "DEL")
                                    (member 'control modifiers)))))
         (literal-predicate (lambda (basic-key modifiers)
                              (and (not (equal basic-key "DEL"))
                                   (member 'control modifiers)))))
    (pcase keypad--modifier
      ('meta         (keypad--filter-keymap-2
                      (keypad--lookup-key (kbd (concat input " ESC")))
                      literal-predicate))
      ('control-meta (keypad--filter-keymap-2
                      (keypad--lookup-key (kbd (concat input " ESC"))) ;; (read-kbd-macro)
                      ctrl-predicate))
      ('literal      (keypad--filter-keymap-2
                      (keypad--lookup-key (kbd input)) ;; (read-kbd-macro)
                      literal-predicate)))))

(defun keypad--keymap-to-describe-entered-keys-with-modifier-3 ()
  "Return a keymap with continuations for prefix keys + modifiers
entered in Keypad. This keymap is intended to be passed further
to Which-key API."
  (let* ((input (keypad--get-entered-keys-as-string))
         ...
         (literal-predicate (lambda (event)
                              (let ((key (single-key-description))
                                ...
                                (and (not (equal key "DEL"))
                                     (member 'control modifiers))))))
    (pcase keypad--modifier
      ('meta (let ((keymap (keypad--lookup-key (kbd (concat input " ESC"))))) ;; (read-kbd-macro)
               (when (keymapp keymap)
                 (let ((result (make-sparse-keymap))) ;; make-keymap
                   (suppress-keymap result t)
                   (map-keymap (lambda (event command)
                                 (when (funcall literal-predicate event)
                                   (define-key result
                                               (keypad--strip-ctrl-meta-from-event event)
                                               command)))
                               keymap)
                   result))))
      ('control-meta (let ((keymap (keypad--lookup-key (kbd (concat input " ESC"))))) ;; (read-kbd-macro)
                       (when (keymapp keymap)
                         (let ((result (make-sparse-keymap))) ;; make-keymap
                           (suppress-keymap result t)
                           (map-keymap (lambda (event command)
                                         (when (funcall ctrl-predicate event)
                                           (define-key result
                                                       (keypad--strip-ctrl-meta-from-event event)
                                                       command)))
                                       keymap)
                           result)))
                     (keypad--filter-keymap-2
                      (keypad--lookup-key (kbd (concat input " ESC"))) ;; (read-kbd-macro)
                      ctrl-predicate))
      ;; ('meta         (keypad--filter-keymap-2
      ;;                 (keypad--lookup-key (kbd (concat input " ESC")))
...
(defun keypad--filter-keymap-2 (keymap predicate)
...
(defun keypad--filter-keymap (keymap &optional control)
  "Return a copy of KEYMAP with only literal — non Ctrl events.
When CONTROL is non-nil leave only Ctrl-... events instead."
  (let ((result (make-sparse-keymap))) ;; make-keymap
    (suppress-keymap result t)
    (map-keymap (lambda (event command)
                  (unless (equal (single-key-description (event-basic-type event))
                                 "DEL") ; DEL
                    (let ((ctrl? (member 'control (event-modifiers event))))
                      (when (or (and control ctrl?)
                                (not ctrl?))
                        (define-key result
                                    (keypad--strip-ctrl-meta-from-event event)
                                    command)))))
  ...
  (let ((result (make-sparse-keymap))) ;; make-keymap
    ...
    (map-keymap (lambda (event command)
                  (let ((key (single-key-description event)))
                    (unless (or (member 'control (event-modifiers event))
                                (member key (list keypad-meta-prefix
                                                  keypad-ctrl-meta-prefix
                                                  keypad-literal-prefix))
                                (alist-get key keypad-start-keys))
                      ;; (let ((k (keypad--strip-ctrl-meta-from-event event)))
                      ;;   (unless (lookup-key keymap k)
                      ;;     (define-key keymap k command)))
                      (define-key result
                                  (keypad--strip-ctrl-meta-from-event event)
                                  command))))
                keypad-leader-map)
             ...
             (ignored (append (list keypad-literal-prefix "DEL")
                              (if (keypad--meta-keybindings-available-p)
                                  (list keypad-meta-prefix
                                        keypad-ctrl-meta-prefix)))))
        (map-keymap (lambda (event command)
                      (when (and (memq 'control (event-modifiers event))
                                 command)
                        (let ((e (keypad--strip-ctrl-meta-from-event event)))
                          (unless (and (member (single-key-description e) ignored)
                                       ;; (lookup-key result e)
                                       )
                            (define-key result e command)))))
                    keymap)
        (map-keymap (lambda (event command)
                      (unless (and (memq 'control (event-modifiers event))
                                   (member (single-key-description event) ignored))
                        (let ((e (keypad--strip-ctrl-meta-from-event event)))
...
Return vector suitable to pass to `define-key'."
  (let ((basic-type (event-basic-type key-event)))
    (vector
     (if (and (integerp basic-type)
