;;; helix-multiple-cursors-core.el --- An experiment in multiple cursors for emacs.
...
(defun mc/store-current-state-in-overlay (o)
  ...
  (overlay-put o 'mark (set-marker (make-marker) (let ((mark-even-if-inactive t))
                                                   (mark))))
...
which action is being undone."
  (when-let ((cursor (mc/cursor-with-id id)))
